                ...
                withCredentials([usernamePassword(credentialsId: 'aws-creds', passwordVariable: 'AWS_SECRET_ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')]) {
                    sh """
                        ...
                        if [ -f /var/jenkins_home/seed.tfstate ]; then
                          echo "Found seed terraform state! Pushing to S3..."
                          ./terraform state push /var/jenkins_home/seed.tfstate || true
        ...
        stage('Deploy to AWS EC2 (SSM)') {
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'aws-creds', passwordVariable: 'AWS_SECRET_ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')]) {
                    ...
                    def geminiApiKey = params.GEMINI_API_KEY?.getPlainText()?.trim() ?: ''
                    def openRouterApiKey = params.OPENROUTER_API_KEY?.getPlainText()?.trim() ?: ''
